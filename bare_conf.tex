...
\author{
\IEEEauthorblockN{{ Mohit Yadav$^{1,2}$, Anil K Sao$^{2}$, A D Dileep$^{2}$ and Padmanabhan Rajan$^{2}$}\\
...
the information-bottleneck (IB) principle \cite{reviewPaper1,aIB2}. In this paper, we focus on the IB principle based approach as it presents a non-parametric solution and also has computational advantages over the
...
Furthermore, these features yields a significant improvement in the diarization accuracy 
on fusion with MFCC. 
